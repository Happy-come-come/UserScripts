...
(async function(){
	...
	async function noteTweetExpander(tweetNodes){
		tweetNodes.forEach(function(target){
			...
			tweetTextsElement.forEach(async (tweetTextElement, index) => {
				if(!(tweetTextElement.innerText.match(/…$/) || (showMoreLinks[index]?.tagName.toLowerCase() == "div"))){
	...
	async function showAllMedias(triggeredUrl){
		...
		async function expand(){
			...
			const madiaRow = mediaPlace.querySelectorAll(`.r-18u37iz.r-9aw3ui.r-14gqq1x.r-1xpp3t0:not(.Show_all_Medias_checked)`);
			...
			mediaNodes.forEach(node => {
				...
				numberSpan.style.left = 'calc(88.5%)';
				numberSpan.style.top = 'calc(87.5%)';
