...
(function() {
	...
	async function main(){
		...
		document.querySelectorAll('span[data-testid="tweet-text-show-more-link"]').forEach(async function(element){
			...
			const hashtags = get_only_particular_key_value(note_tweet.entity_set.hashtags,"text");
			const user_mentions = get_only_particular_key_value(note_tweet.entity_set.user_mentions,"screen_name");
			...
			hashtags.forEach(target =>{
				new_tweet_text = new_tweet_text.replace(new RegExp(`(#|＃)${target}(?=(\\s|$|\\u3000|\\W)(?!\\.|,))`, 'gu'), `<a class="${link_class}" dir="ltr" role="link" href="https://twitter.com/hashtag/${target}">#${target}</a>`);
			});
			user_mentions.forEach(target =>{
				new_tweet_text = new_tweet_text.replace(new RegExp(`@${target}(?=(\\s|$|\\u3000|\\W)(?!\\.|,))`, 'gu'), `<a class="${link_class}" dir="ltr" role="link" href="https://twitter.com/${target}">@${target}</a>`);
			});
			urls.forEach(target =>{
